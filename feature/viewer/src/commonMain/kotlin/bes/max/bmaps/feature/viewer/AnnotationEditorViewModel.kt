...
class AnnotationEditorViewModel(private val repository: AnnotationRepository) : ViewModel() {
    ...
    fun mapEvent(pyramid: TilePyramid, event: MapEvent) {
        ...
        when (event) {
            ...
            is MapEvent.ViewportChanged -> {
                ...
                val nw = WebMercator.geographic(MapPoint(world.left.coerceIn(0.0, 1.0), world.top.coerceIn(0.0, 1.0))) ?: return
                val se = WebMercator.geographic(MapPoint(world.right.coerceIn(0.0, 1.0), world.bottom.coerceIn(0.0, 1.0))) ?: return
            ...
            try {
                ...
                do {
                    when (val result = repository.annotations(id, box, cursor)) {
                        is PackageResult.Failure -> { mutableState.update { it.copy(error = Res.string.annotations_load_error) }; return@launch }
                        is PackageResult.Success -> { items += result.value.items; cursor = result.value.nextCursor }
                    }
                } while (cursor != null && items.size < AnnotationGeoJson.MAX_FEATURES)
                mutableState.update { it.copy(items = items, truncated = cursor != null, error = null) }
            } catch (cancelled: CancellationException) { throw cancelled }
            catch (_: Exception) { mutableState.update { it.copy(error = Res.string.annotations_load_error) } }
        }
    }

    fun panel(show: Boolean) { if (!state.value.busy) mutableState.update { it.copy(panel = show) } }
    fun retry() = refresh()
    fun visibility(kind: AnnotationKind, visible: Boolean) {
        managers.first { it.kind == kind }.visibility(visible)
        mutableState.update { it.copy(layers = managers.map { layer -> layer.state }, selected = it.selected?.takeUnless { item -> item.kind == kind && !visible }) }
    }
    fun select(id: String, position: MapPoint? = null) {
        if (state.value.busy) return
        if (state.value.draft != null) {
            val index = id.takeIf { it.startsWith("vertex:") }?.removePrefix("vertex:")?.toIntOrNull()
        ...
        mutableState.update { it.copy(selected = value, layers = managers.map { layer -> layer.state }, panel = false) }
    }
    fun dismissSelection() {
        managers.forEach { it.select(null) }
        mutableState.update { it.copy(selected = null, deleteConfirmation = false, layers = managers.map { layer -> layer.state }) }
    }
    fun start(kind: AnnotationKind) {
        if (state.value.busy) return
        managers.forEach { it.select(null) }
        mutableState.update { it.copy(layers = managers.map { layer -> layer.state }, draft = Annotation(kind = kind, coordinates = emptyList()), history = emptyList(), panel = false,
            selected = null, propertiesOpen = false, replacingVertex = null, error = null) }
    }
    fun edit() {
        val value = state.value.selected ?: return
        mutableState.update { it.copy(draft = value, selected = null, history = emptyList(), propertiesOpen = true, replacingVertex = null) }
    }
    fun addPoint(point: GeographicCoordinate) {
        ...
        val points = when {
            ...
            else -> { channel.trySend(Res.string.annotations_vertex_limit); return }
        }
    ...
    private fun change(value: Annotation) {
        ...
        mutableState.update { it.copy(draft = value, history = (it.history + previous).takeLast(1000), error = null) }
    }
    fun name(value: String) { state.value.draft?.let { change(it.copy(name = value.take(120))) } }
    fun description(value: String) { state.value.draft?.let { change(it.copy(description = value.take(4000))) } }
    fun color(value: String) { if (Regex("#[0-9a-fA-F]{6}").matches(value)) state.value.draft?.let { change(it.copy(color = value)) } }
    fun icon(value: String) { if (MarkerIcons.entries.any { it.id == value }) state.value.draft?.let { change(it.copy(icon = value)) } }
    fun undo() {
        if (state.value.busy) return
        val previous = state.value.history.lastOrNull() ?: return
        mutableState.update { it.copy(draft = previous, history = it.history.dropLast(1), replacingVertex = null, error = null) }
    }
    fun removeVertex(index: Int) { state.value.draft?.let { value -> change(value.copy(coordinates = value.coordinates.filterIndexed { i, _ -> i != index })) } }
    ...
    fun properties(show: Boolean) { mutableState.update { it.copy(propertiesOpen = show, replacingVertex = null) } }
    fun cancel() {
        if (state.value.busy) return
        mutableState.update { it.copy(draft = null, history = emptyList(), propertiesOpen = false, replacingVertex = null, error = null) }
    }
    fun save() {
        val draft = state.value.draft ?: return
        if (AnnotationValidation.error(draft) != null) { mutableState.update { it.copy(error = Res.string.annotations_invalid) }; return }
        mutate { id -> repository.saveAnnotations(id, listOf(draft)) }
    }
    fun confirmDelete(show: Boolean) { if (!state.value.busy) mutableState.update { it.copy(deleteConfirmation = show) } }
    fun delete() { val value = state.value.selected ?: return; mutate { repository.deleteAnnotation(it, value.id) } }
            ...
            try {
                when (action(id)) {
                    is PackageResult.Success -> {
                        pendingImport = null
                        managers.forEach { it.select(null) }
                        mutableState.update { it.copy(draft = null, history = emptyList(), selected = null, propertiesOpen = false,
                            deleteConfirmation = false, geoJsonOpen = false, geoJson = "", replacingVertex = null, layers = managers.map { layer -> layer.state }) }
                        refresh()
                    }
                    is PackageResult.Failure -> channel.send(Res.string.annotations_save_error)
                }
            } catch (cancelled: CancellationException) { throw cancelled }
            catch (_: Exception) { channel.send(Res.string.annotations_save_error) }
            finally { if (packageId == id) mutableState.update { it.copy(busy = false) } }
        }
    }
    fun geoJson(show: Boolean) {
        if (!state.value.busy) mutableState.update { it.copy(geoJsonOpen = show, geoJson = "", geoJsonExport = false, panel = false) }
    }
    fun geoJsonText(value: String) {
        if (!state.value.busy && value.length <= AnnotationGeoJson.MAX_BYTES) mutableState.update { it.copy(geoJson = value, error = null) }
    }
    fun importGeoJson() {
        ...
        mutate { id ->
            val values = try {
                pendingImport?.takeIf { it.first == text }?.second ?: withContext(Dispatchers.Default) {
                    AnnotationGeoJson.decode(text).also { require(it.isNotEmpty()); require(it.all { value -> AnnotationValidation.error(value) == null }) }
    ...
    fun exportGeoJson() {
        ...
        mutation = viewModelScope.launch {
            try {
                ...
                var cursor: String? = null
                do {
                    val page = (repository.annotations(id, after = cursor) as? PackageResult.Success)?.value ?: error("Read failed")
                    values += page.items
                ...
                mutableState.update { it.copy(geoJson = text, geoJsonOpen = true, geoJsonExport = true, panel = false) }
            } catch (cancelled: CancellationException) { throw cancelled }
            catch (_: Exception) { channel.send(Res.string.annotations_export_error) }
            finally { if (packageId == id) mutableState.update { it.copy(busy = false) } }
        }
    }
}
