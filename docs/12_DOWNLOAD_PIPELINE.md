...
The constructor can add provider/style layers to the selected area. Every layer uses the same selected zoom levels, with presentation validation against each provider’s coverage and tile matrix. Combined estimates include all layers, including hidden layers. Each layer has its own 300 MB limit; `estimatedLargestLayerBytes` drives the size warning/submission gate while `estimatedPackageBytes` drives combined size display and free-space checks. The root writes to `map_data.mbtiles`; each additional layer writes to `layers/<layer-id>.mbtiles`. Initial visibility and opacity are persisted in the durable request and manifest.
