...
The application limit is **300,000,000 bytes per raster tile layer**, including that layer’s MBTiles database overhead. There is no aggregate package limit; elevation, annotations, and metadata are excluded from the layer allowance. `PackageSizePolicy.maxLayerBytes` records the requested per-layer limit, clamped to the application maximum. The legacy version-1 field `maxBytes` is accepted as a deserialization alias and interpreted per layer; new manifests and durable requests write `maxLayerBytes`. Missing policies retain the 300 MB per-layer default. Existing manifests and resumable jobs remain readable. Download `receivedBytes` measures network payload; `packageBytes` measures actual package storage including database/manifest overhead. They are not interchangeable. Temporary staging, SQLite journals, and export copies also require additional free-space checks. Writers enforce the effective layer limit transactionally and check each layer during finalization/opening; `SizeLimitExceeded` never silently drops tiles. Total estimates and actual package bytes remain available for capacity checks and display. Manifest read/write bounds remain independent format constraints.
...
`BuildLayerRequest.zoomLevels` and `PackageLayer.zoomLevels` preserve exact levels, including legacy non-contiguous selections. New constructor downloads require a continuous inclusive range. Empty sets retain the original inclusive-range semantics. Final layers record their verified `tileCount`; a null count identifies a draft or a package requiring import validation. New package writes serialize defaulted `elevationDataset` (`NONE` for legacy/tile-only requests). Successful optional DEM downloads populate `elevation` with the `elevation.geotiff` asset; tile-only packages retain `elevation: null`. See `16_ELEVATION_DOWNLOADS.md`. Summaries expose elevation availability and durable tile counters; bounds may be null for corrupt orphan entries without readable metadata.
