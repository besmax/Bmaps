...
Polygon persistence fix, 2026-09-20: append the closing GeoJSON position as a single array element. Adding a `JsonArray` directly to the vertex list selected Kotlin's iterable overload and appended two scalar numbers, making saved polygons unreadable on refresh. `AnnotationGeoJsonTest` reproduced the failure before the fix; all three tests pass with `:domain:map-builder:testAndroidHostTest --tests '*AnnotationGeoJsonTest'` afterward. The shared storage scenario now checks polygon viewport queries and persistence across reopening; execution of that native scenario and visual acceptance remain pending. Previously written malformed polygon rows are recovered on read: the storage decoder recognizes only the old flattened closing position when its two numeric entries exactly match the first position, reconstructs the ring, and applies normal geometry validation. The repository uses this decoder so a legacy polygon no longer blocks the complete object page. External GeoJSON imports remain strict. Recovery does not rewrite the database; subsequent saves and exports use valid GeoJSON. All five codec tests pass, covering legacy recovery, preserved properties, strict imports, and rejection of unrelated corruption. The shared storage/reopen scenario now seeds a legacy polygon alongside markers; its native execution remains pending.
