...
* **`core:storage`**
    ...
    * **Phase 4:** Private staging/final directories, bounded streaming, synchronization, capacity/size checks, path safety, and cleanup. Depends on `core:di` for platform location bindings and the catalog-pinned bundled SQLite driver for annotation databases. Bounded streamed DEM writes, temporary-file cleanup, and atomic asset commits are implemented. DEM parsing and transfer IO remain future work.
* **`core:map-engine`**
