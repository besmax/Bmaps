...
`RasterMap` owns one engine session per configuration/layer list/container size. It applies fit-relative scale after layout, forwards gestures through callbacks, and recreates the session on input or size changes. Consequently resize/recreation restores the supplied initial viewport; durable viewport retention is a later presentation responsibility. Callers should retain a stable layer list and create fresh source instances in its factories.
...
Each returned RawSource contains complete in-memory tile bytes. It owns no socket/file handle; MapComposeMP closes it after decoding. PNG/JPEG signature, encoded size (2,000,000 bytes), and exact tile dimensions are checked before decode. Android uses BitmapFactory; iOS uses Skia. A bounded validation decode catches malformed images before returning a stream. This currently performs an extra decode; revisit with profiling before adding caches. Missing tiles return null; corrupt or failed tiles emit `TileFailed` first. The internal event channel is bounded and nonblocking; it is not durable event delivery.
